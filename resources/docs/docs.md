...
The `home-routes` handler in the `image-detection-api.routes.home` namespace
...
The `home-page` function will in turn call the `image-detection-api.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `image-detection-api.handler` namespace:
...
Request middleware functions are located under the `image-detection-api.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `image-detection-api.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `image-detection-api.db.core` in some other namespace.
